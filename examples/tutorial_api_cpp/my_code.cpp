...
// Producer
DEFINE_string(video_file,                "examples/media/basketball/VID_20190614_124235.mp4",
...
{
public:
    WUserInput(const std::string& videoFile) :
        mVideoFile{videoFile}, // For all basic image formats
        // If we want only e.g., "jpg" + "png" images
        // mImageFiles{op::getFilesOnDirectory(directoryPath, std::vector<std::string>{"jpg", "png"})},
        mVideoCapture(cv::VideoCapture(videoFile))
    {
        if (!mVideoCapture.isOpened())
            op::error("can't open video: " + mVideoFile, __LINE__, __FUNCTION__, __FILE__);
    }
        ...
        try
        {
            if (datumsPtr != nullptr && !datumsPtr->empty())
                for (auto& datumPtr : *datumsPtr) {
                    ...
                    // cv::putText(datumPtr->cvOutputData, "hi", cv::Point(25,25), cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0,0,0));
                }
        }
        catch (const std::exception& e)
        {
        ...
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                ...
                if (!FLAGS_no_display)
                {
                    ...
                    const char key = (char)cv::waitKey(1000);
                    if (key == 27)
                        this->stop();
                }
            }
...
void configureWrapper(op::WrapperT<UserDatum>& opWrapperT)
{
    try
    {
        ...
        auto wUserInput = std::make_shared<WUserInput>(FLAGS_video_file);
        // Processing
